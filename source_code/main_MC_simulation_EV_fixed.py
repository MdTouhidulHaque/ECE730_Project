...
import EV_Profile_Allocation_MC_Simulation_EV_Fixed
...
for iRandom in range(Num_Run): # Starts the loop according to the number of simulations to perform in the Monte Carlo study
    ...
    for selected_day in range(30):
        ...
        EV_penetration_list = [penetration_list[0]]
        EVcustomer_number_PV = EVcustomer_number[0]
        for iPenetration in range(len(EV_penetration_list)):
            EV_Profile_Allocation_MC_Simulation_EV_Fixed.EV_Profile_Allocation_MC_Simulation(EVcustomer_number_PV, EV_penetration_list, iPenetration, Loadname, iRandom, Definitions.DSSCircuit, Definitions.DSSText, home_ev_charging_demand_profile, no_EV_Profiles)
        ...
        random.seed(iRandom)
        for iPenetration in range(len(penetration_list)):     
            PV_Profile_Allocation_MC_Simulation.PV_Allocation_MC_Simulation(PVcustomer_number, penetration_list, iPenetration, PVname, iRandom, inverter_op, PV_allocation_size, Definitions.DSSCircuit, Definitions.DSSText)
...
plt.title('Impact of PV Generation with Fixed EV Charging Loads on Voltage Profile of Consumers', fontsize = 24, fontweight='bold')
plt.ylabel('Voltage Non-Compliant Consumers, %', fontsize = 20, fontweight='bold')
plt.ylim([0, 120])
plt.xlabel('PV Penetration Level, %', fontsize = 20, fontweight='bold')
...
for penetration_level in range(0, 101, 10):  # Adjust the range as needed
    ...
    plt.title(f'Voltage Profile for Each HV Node of the Distribution System at {penetration_level}% PV Penetration', fontsize=16, fontweight='bold')
...
for penetration_level in range(0, 101, 10):  # Adjust the range as needed
    ...
    plt.title(f'Utilization Level for Each Distribution Transformer at {penetration_level}% PV Penetration', fontsize=18, fontweight='bold')
